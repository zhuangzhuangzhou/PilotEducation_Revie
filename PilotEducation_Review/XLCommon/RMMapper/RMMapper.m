...
+(id)populateObject:(id)obj fromDictionary:(NSDictionary *)dict exclude:(NSArray *)excludeArray {
    ...
    for (id key in dict) {
        ...
        if ([key isKindOfClass:[NSString class]] == NO) {
            DLog(@"TDUtils: key must be NSString. Received key %@", key);
            continue;
        }
        
        // If key is not inside the object properties, skip it
        if ([properties objectForKey:key] == nil) {
            DLog(@"TDUtils: key %@ is not existed in class %@", key, NSStringFromClass(cls));
            continue;
        }
            ...
            else if ([value isKindOfClass:[NSArray class]])
            {
                ...
                for (id data in value) {
                    if ([data isKindOfClass:[NSDictionary class]]) {
                        NSString * nowKey = (NSString *)key;
                        NSString * currentClassName = [self uppeFirstChar:[nowKey substringToIndex:nowKey.length - 1]];
                        Class objCls = NSClassFromString(currentClassName);
                        id childObjr = [[objCls alloc] init];
                        
                        [RMMapper populateObject:childObjr fromDictionary:data exclude:nil];
                        
                        [obj setValue:childObjr forKey:key];
                    }
                    else
                    {
                        isDic = NO;
...
+ (NSString *) uppeFirstChar:(NSString *) origin
{
    NSMutableString * str = [[NSMutableString alloc] init];
    [str appendString:[[origin substringToIndex:1] uppercaseString]];
    [str appendString:[origin substringFromIndex:1]];
    return str;
}
