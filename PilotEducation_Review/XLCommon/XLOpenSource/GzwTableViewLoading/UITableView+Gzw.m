...
{
    objc_setAssociatedObject(self, &dataVerticalOffsetKey,@(dataVerticalOffset),OBJC_ASSOCIATION_RETAIN);// 如果是对象，请用RETAIN。
}
...
//        [activityView startAnimating];
        ...
        //创建一个webView，添加到界面
        UIWebView *webView = [[UIWebView alloc] initWithFrame:CGRectMake(([UIScreen mainScreen].bounds.size.width - 100) / 2, ([UIScreen mainScreen].bounds.size.height - 64 - 49 - 80) / 2 + 20, 100, 80)];
        //自动调整尺寸
...
- (NSAttributedString *)buttonTitleForEmptyDataSet:(UIScrollView *)scrollView forState:(UIControlState)state
{
    if (self.loading) {
        ...
        return [[NSAttributedString alloc] initWithString:self.buttonText ? self.buttonText : @"重试" attributes:attributes];
    }
}
// 返回视图的垂直位置（调整整个试图的垂直位置）
